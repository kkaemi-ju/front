      ...
      <section class="relative h-[500px] overflow-hidden">
        ...
        >
          ...
          >
            ...
            <div class="container mx-auto relative h-full flex items-center justify-center text-white px-4">
              <div class="text-center space-y-4">
                <h1 class="text-4xl font-bold">{{ slide.title }}</h1>
                <p class="text-lg text-[#D5ED9F]">{{ slide.subtitle }}</p>
                <div class="flex justify-center gap-4 mt-8">
                  <button class="px-4 py-2 bg-[#FF9100] hover:bg-[#FF9100]/90 text-white rounded">여행지 찾기</button>
                  <button class="px-4 py-2 border border-white text-white hover:bg-white/20 rounded">더 알아보기</button>
                </div>
              </div>
            </div>
          </div>
      ...
      <section class="py-12 bg-white">
        <div class="container mx-auto max-w-screen-lg px-4">
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <router-link v-for="category in categories" :key="category.name" :to="category.link" class="block mb-4">
              <div class="bg-gray-100 h-24 md:h-32 flex items-center justify-center hover:bg-gray-200 transition-colors">
      ...
      <section class="py-12 bg-[#FFFBE6]">
        <div class="container mx-auto px-4">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div v-for="spot in hotSpots" :key="spot.title" class="overflow-hidden hover:shadow-lg transition-shadow bg-white rounded-lg">
              <img
                :src="spot.image"
                :alt="spot.title"
              ...
              <div class="p-4">
                ...
                <p class="text-sm text-gray-600 mt-2">특별한 순간을 만나보세요</p>
              </div>
            </div>
          </div>
        </div>
